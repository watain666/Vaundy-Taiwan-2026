...
正式建置會直接輸出首頁 HTML，內嵌首頁所需樣式，並在背景下載完整樣式表；首次操作或進入其他頁面時才套用完整樣式。互動程式正常啟動，不需要等第一次點擊或捲動才載入。直接開啟 `#/guide`、`#/setlist` 或歌曲連結時，會先隱藏首頁，待完整樣式就緒後再顯示指定頁面，避免閃爍與版面跳動。首頁與頁面切換共用 `src/ui/home.js`，請不要另外手寫一份首頁快照。
